  ...
  const handleSaveInstruments = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedInstruments.length; i++) {
        ...
        const response0 = await fetch(
          `https://www.stringsattached.online/hci/api/api/default/groups?name=${inst}`,
          {
            method: "GET",
            headers: {
              "Authorization": `Bearer ${token}`,
    ...
    console.log(response3.json());
      ...
      const response5 = await fetch(
        `https://www.stringsattached.online/hci/api/api/default/group-members?userID=${userID}&groupID=${GID}`,
        {
          method: "GET",
          headers: {
            "Authorization": `Bearer ${token}`,
