...
const UserProfilePage = () => {
  ...
  if (conexist === false) {
    const response2 = await fetch(
      `https://stringsattached.online/hci/api/api/default/connections`,
      {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
...
let buttonStyle;
  ...
  if (isFollowing === 'follow') {
    ...
    const checkIfBlocked = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                `https://stringsattached.online/hci/api/api/default/connections?fromUserID=${currentUserId}&toUserID=${userID}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
    ...
    const fetchPostReactions = useCallback(async (postId) => {
        ...
        try {
            const response = await fetch(
                `https://stringsattached.online/hci/api/api/default/post-reactions?postID=${postId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
        ...
        try {
            ...
            if (existingConnection) {
                ...
                const patchResponse = await fetch(
                    `https://stringsattached.online/hci/api/api/default/connections/${existingConnection.id}`,
                    {
                        method: "PATCH",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const fetchCommentsForPost = async (postId) => {
        ...
        try {
            const response = await fetch(
                `https://stringsattached.online/hci/api/api/default/posts?parentPostID=${postId}`,
                {
                    headers: {
                        "Authorization": `Bearer ${token}`,
                    },
    ...
    const handleReaction = async (postId, reactionType) => {
        ...
        try {
            const existingResponse = await fetch(
                `https://stringsattached.online/hci/api/api/default/post-reactions?postID=${postId}&reactorID=${currentUserId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            if (existingReaction?.name === reactionType) {
                await fetch(
                    `https://stringsattached.online/hci/api/api/default/post-reactions/${existingReaction.id}`,
                    {
                        method: "DELETE",
                        headers: {
                            Authorization: `Bearer ${token}`,
                ...
                await fetch(
                    `https://stringsattached.online/hci/api/api/default/post-reactions`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
