  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const fetchResponse = await fetch(
        `https://webdev.cse.buffalo.edu/hci/api/api/claem/file-uploads?uploaderID=${userID}`,
        {
          method: "GET",
          headers: {
      ...
      const deleteResponse = await fetch(
        `https://webdev.cse.buffalo.edu/hci/api/api/claem/file-uploads/${latestImage.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authToken}`,
    ...
    setShowDeleteImageModal(false);
      ...
      if (secondLatestImage) {
        const imageUrl = `https://webdev.cse.buffalo.edu/${secondLatestImage.path}`;
      ...
      const reviewerData = await Promise.all(
        reviews.map(async (review) => {
          const reviewerRes = await fetch(
            `https://webdev.cse.buffalo.edu/hci/api/api/claem/users/${review.reviewerID}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
...
const fetchFollowedUsers = async () => {
  ...
  try {
    const response = await fetch(
      `https://webdev.cse.buffalo.edu/hci/api/api/claem/connections?fromUserID=${userID}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    const pendingFetches = pendings.map(async (conn) => {
      try {
        const res = await fetch(
          `https://webdev.cse.buffalo.edu/hci/api/api/claem/users/${conn.toUserID}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const uploadImage = async (file) => {
    ...
    try {
      ...
      return { url: `https://webdev.cse.buffalo.edu/${data.path}`, fileId: data.id };
    } catch (error) {
...
const deleteAccount = async () => {
  ...
  try {
      const response = await fetch(
          `https://webdev.cse.buffalo.edu/hci/api/api/claem/users/${storedUserID}?relatedObjectsAction=delete`, 
          {
              method: "DELETE",
              headers: {
                  "Authorization": `Bearer ${token}`,  
...
const fetchUserProfile = useCallback(async () => {
  ...
  try {
      const response = await fetch(
          `https://webdev.cse.buffalo.edu/hci/api/api/claem/users/${storedUserID}`,
          {
              method: "GET",
              headers: {
      ...
      if (!imageUrl || imageUrl === "undefined" || imageUrl === "") {
        ...
        const fullPath = isFullUrl ? imageUrl : `https://webdev.cse.buffalo.edu/${imageUrl}`;
...
//
//       const deleteResponse = await fetch(
//         `https://webdev.cse.buffalo.edu/hci/api/api/claem/posts/${post.id}`,
//         {
//           method: "DELETE",
//           headers: {
//             "Authorization": `Bearer ${authToken}`,
...
const handleFileChange = async (e) => {
  ...
  try {
    ...
    const newImageUrl = `https://webdev.cse.buffalo.edu/${data.path}`;
...
const updateAge = async () => {
  ...
  const deletePost = async (postId, fileId = null) => {
    ...
    try {
      const deleteResponse = await fetch(
        `https://webdev.cse.buffalo.edu/hci/api/api/claem/posts/${postId}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${authToken}`,
        ...
        {blockedUsers.map((user) => {
          const profileImage =
            ...
            user.toUser?.attributes?.profileImage !== "undefined"
              ? `https://webdev.cse.buffalo.edu/${user.toUser.attributes.profileImage}`
              : noIcon;
  ...
  <ul style={{ listStyle: "none", padding: 0 }}>
    {pendingReviews.map((review) => {
      const imageSrc = review.profileImage && review.profileImage !== "undefined"
        ? `https://webdev.cse.buffalo.edu/${review.profileImage}`
        : noIcon;
