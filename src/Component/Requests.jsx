...
const Requests = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://www.stringsattached.online/api/api/default/connections?toUserID=${userId}`, {
      method: "GET",
      headers: {
  ...
  const acceptConnection = async (id) => {
    const token = sessionStorage.getItem("token");
    await fetch(`https://www.stringsattached.online/api/api/default/connections/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
